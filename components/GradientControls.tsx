...
const createGradientValue = (
  ...
  color1: string,
  color2: string,
): string => `linear-gradient(${direction}deg, ${color1} 0%, ${color2} 100%)`;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span className="text-[10px] md:text-xs text-white/90 drop-shadow-md font-medium leading-none">
            Custom
          </span>
        </div>
      </button>
        ...
        <div className="col-span-2 space-y-3 mt-1 p-3 bg-secondary/50 rounded-lg border border-border/40">
          ...
          {customThemes.length > 0 && (
            <div className="space-y-2">
              <span className="text-xs font-medium text-foreground/80 mt-2 block">
                Saved Themes
              </span>
              <div className="grid grid-cols-5 md:grid-cols-3 gap-2 md:gap-3">
                {customThemes.map((theme) => {
                  const themeGradient = createGradientValue(
                    theme.direction,
                    theme.color1,
                    theme.color2,
                  );
                  const isActive =
                    ...
                    state.background.value === themeGradient;

                  return (
                    <div
                      key={theme.id}
                      className="relative group/theme flex flex-col items-center gap-1 md:gap-1.5"
                    >
                      <button

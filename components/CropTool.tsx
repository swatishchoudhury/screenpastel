...
import { Check, X, RectangleHorizontal, ChevronDown } from "lucide-react";
...
export default function CropTool({ imageSrc, onApply, onCancel }: CropToolProps) {
  ...
  const applyAspectRatio = useCallback(
    (c: CropRegion, ratio: number | null, anchor: "center" | "top-left" = "center"): CropRegion => {
      if (!ratio) return c;
      let { x, y, width, height } = c;
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      if (activeHandle === "move") {
        const newX = Math.max(0, Math.min(cropStart.x + dx, imgRect.width - cropStart.width));
        const newY = Math.max(0, Math.min(cropStart.y + dy, imgRect.height - cropStart.height));
        setCrop({ x: newX, y: newY, width: cropStart.width, height: cropStart.height });
        return;
      }
      ...
      if (!aspectRatio) {
        ...
        if (activeHandle.includes("left")) {
          ...
          if (x < 0) { width += x; x = 0; }
          if (width < 20) { x = cropStart.x + cropStart.width - 20; width = 20; }
        }
        if (activeHandle.includes("right")) {
          width = cropStart.width + dx;
          if (x + width > imgRect.width) { width = imgRect.width - x; }
        ...
        if (activeHandle.includes("top")) {
          ...
          if (y < 0) { height += y; y = 0; }
          if (height < 20) { y = cropStart.y + cropStart.height - 20; height = 20; }
        }
        if (activeHandle.includes("bottom")) {
          height = cropStart.height + dy;
          if (y + height > imgRect.height) { height = imgRect.height - y; }
      ...
      if (isCorner) {
        ...
        const heightFromDy = activeHandle.includes("top") ? cropStart.height - dy : cropStart.height + dy;
        ...
        const desiredHeight = activeHandle === "top" ? cropStart.height - dy : cropStart.height + dy;
        desiredWidth = desiredHeight * aspectRatio;
      } else {
        desiredWidth = activeHandle === "left" ? cropStart.width - dx : cropStart.width + dx;
      }
      ...
      if (w > imgRect.width) { w = imgRect.width; h = w / aspectRatio; }
      if (h > imgRect.height) { h = imgRect.height; w = h * aspectRatio; }

      if (activeHandle.includes("left")) {
        if (w > anchorX) { w = anchorX; h = w / aspectRatio; }
      } else if (activeHandle.includes("right")) {
        if (anchorX + w > imgRect.width) { w = imgRect.width - anchorX; h = w / aspectRatio; }
      }

      if (activeHandle.includes("top")) {
        if (h > anchorY) { h = anchorY; w = h * aspectRatio; }
      } else if (activeHandle.includes("bottom")) {
        if (anchorY + h > imgRect.height) { h = imgRect.height - anchorY; w = h * aspectRatio; }
      }
        ...
        {imgLoaded && (
          <>
            ...
            >
              ...
              <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity: activeHandle ? 0.5 : 0.25 }}>
                <line x1={thirdW} y1={0} x2={thirdW} y2={crop.height} stroke="white" strokeWidth="0.5" />
                <line x1={thirdW * 2} y1={0} x2={thirdW * 2} y2={crop.height} stroke="white" strokeWidth="0.5" />
                <line x1={0} y1={thirdH} x2={crop.width} y2={thirdH} stroke="white" strokeWidth="0.5" />
                <line x1={0} y1={thirdH * 2} x2={crop.width} y2={thirdH * 2} stroke="white" strokeWidth="0.5" />
              </svg>

              <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
                <path d="M 0 12 L 0 0 L 12 0" fill="none" stroke="white" strokeWidth="3" />
                <path d={`M ${crop.width - 12} 0 L ${crop.width} 0 L ${crop.width} 12`} fill="none" stroke="white" strokeWidth="3" />
                <path d={`M 0 ${crop.height - 12} L 0 ${crop.height} L 12 ${crop.height}`} fill="none" stroke="white" strokeWidth="3" />
                <path d={`M ${crop.width - 12} ${crop.height} L ${crop.width} ${crop.height} L ${crop.width} ${crop.height - 12}`} fill="none" stroke="white" strokeWidth="3" />
              </svg>
              ...
              <div className="absolute -bottom-7 left-1/2 -translate-x-1/2 bg-black/70 text-white text-[10px] font-mono px-2 py-0.5 rounded-full whitespace-nowrap pointer-events-none">
                {Math.round(crop.width * (naturalSize.width / imgRect.width))} × {Math.round(crop.height * (naturalSize.height / imgRect.height))}
              </div>
            </div>

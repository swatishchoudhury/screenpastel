...
export default function ColorSwatch({ value, onChange, label }: ColorSwatchProps) {
    const colorInputRef = useRef<HTMLInputElement>(null);
    const [hexInput, setHexInput] = useState(value.toUpperCase().replace("#", ""));
    const [isFocused, setIsFocused] = useState(false);

    const displayHex = isFocused ? hexInput : value.toUpperCase().replace("#", "");
        ...
        <div className="space-y-2">
            ...
            <div className="flex items-center gap-0 bg-secondary/40 border border-border/50 rounded-md p-1 focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 transition-all hover:bg-secondary/60 w-[108px]">
                ...
                <div className="flex items-center gap-0 flex-1 w-full">
                    <span className="text-[11px] text-muted-foreground select-none font-mono">#</span>
                    <input
                        type="text"
                        value={displayHex}
                        onChange={(e) => {
